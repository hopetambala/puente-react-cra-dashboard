    ...
    {({ loading, error, data, refetch, networkStatus }) => {
      ...
      return (
        <>
          <Button
            variant="contained"
            style={{ backgroundColor: styles.theme.lighter_darkbg }}
          >
            {console.log(data)}
            <CSVLink data={data["getPeopleByOrganization"]}>Download</CSVLink>
          </Button>
      ...
      return (
        <>
          <Button style={styles.button}>
            {console.log(data)}
            <CSVLink data={data.getVitalByOrganization}>Download</CSVLink>
          </Button>
    ...
    {({ loading, error, data, refetch, networkStatus }) => {
      ...
      return (
        <>
          <Button style={styles.button}>
            {console.log(data)}
            <CSVLink data={data.getEnvByOrganization}>Download</CSVLink>
          </Button>
    ...
    {({ loading, error, data, refetch, networkStatus }) => {
      ...
      return (
        <>
          ...
          >
            {console.log(data)}
            <CSVLink data={data.getEvalMedicalByOrganization}>Download</CSVLink>
          </Button>
    ...
    {({ loading, error, data, refetch, networkStatus }) => {
      ...
      return (
        <>
          <Button style={styles.button}>
            {console.log(data)}
            <CSVLink data={data.getMedHistoryByOrganization}>Download</CSVLink>
          </Button>
...
const CustomData = (props) => {
  ...
  useEffect(() => {
    ...
    client
      .query({ query: allCustomResultsByFormId, variables: { id } })
      .then(async (res) => {
  ...
  async function clean_data(data) {
    ...
    for (let i = 0; i < cleaned_data["getCustomFormResultsbyId"].length; i++) {
      for (
        let j = 0;
        j < cleaned_data["getCustomFormResultsbyId"][i]["fields"].length;
        j++
      ) {
        var punctRE =
          /[\u2000-\u206F\u2E00-\u2E7F\\'!"#$%&()*+,\-.:;<=>?@_`{|}~]/g;
        ...
        var question = String(
          cleaned_data["getCustomFormResultsbyId"][i]["fields"][j].title
        );
        question = question.replace(punctTitleRE, "");
        var answer = String(
          cleaned_data["getCustomFormResultsbyId"][i]["fields"][j].answer
        );
  ...
  return (
    <>
      {data !== null && (
        <>
          <Button style={styles.button}>
            <CSVLink data={data["getCustomFormResultsbyId"]}>Download</CSVLink>
          </Button>
...
const AssetData = (props) => {
  ...
  async function clean_data(data) {
    ...
    for (let i = 0; i < cleaned_data["getAssetSuppById"].length; i++) {
      for (
        let j = 0;
        j < cleaned_data["getAssetSuppById"][i]["fields"].length;
        j++
      ) {
        var punctRE =
          /[\u2000-\u206F\u2E00-\u2E7F\\'!"#$%&()*+,\-.:;<=>?@_`{|}~]/g;
        ...
        var question = String(
          cleaned_data["getAssetSuppById"][i]["fields"][j].title
        );
        question = question.replace(punctTitleRE, "");
        var answer = String(
          cleaned_data["getAssetSuppById"][i]["fields"][j].answer
        );
        answer = answer.replace(punctRE, "");
  ...
  return (
    <div>
      {data && (
        <div>
          <Button style={styles.button}>
            <CSVLink data={data?.getAssetSuppById}>Download</CSVLink>
          </Button>
    ...
    <>
      <Styles style={styles.container}>
        ...
        <Form
          ...
          initialValues={{
            type: "Demographics",
            organization: props.authInfo.organization,
          }}
            ...
            >
              <div>
                ...
                <Field name="type" component="select">
                  ...
                  <option value="Medical Evaluation">
                    Dem + Medical Evaluation
                  </option>
                  <option value="Environmental Health">
                    Dem + Environmental Health
                  </option>
              ...
              {values.type === "Custom" && (
                <div>
                  ...
                  <Field name="objectId" component="select">
                    <Query query={allCustomSpecs} notifyOnNetworkStatusChange>
                        ...
                        return (
                          <>
                            ...
                            {data.getCustomFormSpec
                              ...
                              .filter((opt) =>
                                opt.typeOfForm.includes("Custom")
                              )
                        ...
                        return (
                          <>
                            ...
                            {data.getCustomFormSpec
                              .sort((a, b) => a.name.localeCompare(b.name))
                              .filter((opt) =>
                                opt.typeOfForm.includes("Assets")
                              )
                              .map((opt) => (
                                <option key={opt.objectId} value={opt.objectId}>
                                  {opt.name}
      ...
      <div style={{ margin: "20px" }}>{aThing}</div>
    </>
  );
};
